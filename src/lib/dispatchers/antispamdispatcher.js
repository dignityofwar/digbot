...
module.exports = class AntiSpamDispatcher extends Dispatcher {
     ...
     */
    constructor({ discordjsClient, moderatorsMutemoderator }) {
        super();
    ...
    async start() {
        ...
        this.client.on('guildMemberAdd', this.joinCheck.bind(this));
    }
    ...
    async handler(message) {
        ...
        if (message.mentions.everyone || message.mentions.members.length || message.mentions.roles.length) {
            // Message has triggered a ping
            this.mutemoderator.hitMember(message.member);
        }
    }

    /**
     * Send a message that a member is on thin ice and shouldn't send out too many mentions
     *
     * @param message
     * @return {Promise<void>}
     */
    async sendWarning(message) {
        // TODO: Maybe do something that it will give different messages if 2 members are being warned shortly after
        //  each other
        await message.reply('do not mention again. I\'ve sent you a DM that I strongly suggest you read now.');
    }
    ...
    joinCheck(member) {
        if (this.mutemoderator.shouldBeMuted(member)) {
            this.mutemoderator.muteGuildMember(member);
        }
    }
...
//
// /**
//  * Confirm if the message actually contains a mention at the specified index, if it does return an identifying portion
//  *
//  * @param message
//  * @param mentionIndex
//  * @return {*}
