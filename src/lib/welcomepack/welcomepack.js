...
module.exports = {
    ...
    message:
        ...
        '\n' +
        '\nFor the best Discord experience, make sure you download the Desktop client: ' + 'https://discordapp.com/api/download?platform=win (windows). We also recommend that you ' +
        'turn on the "Game Activity" option within the desktop version, which will grant you access ' +
        'to our game specific channels automatically. More info on this can be found here: ' + 'https://support.discordapp.com/hc/en-us/articles/217960107-Games-Detection-101' +
        '\n' +
...
function duplicationCheck(mem) {
    for (let i = 0; i < recentJoiners.length; i++) {
...
function sendLot(mem) {
    ...
    mem.sendMessage(module.exports.message)
        .then(
            logger.info(TAG, `Sent welcome message to: ` + (mem.displayName) +
                ', ID: ' + mem.user.id)
        )
        .catch(err => {
