...
module.exports = {
    execute: function(received) {
        ...
        if (received.add) {
            let add = guild.roles.find('name', received.add);
            if (!add) {
        ...
        if (received.remove) {
            let remove = guild.roles.find('name', received.remove);
            if (!remove) {
...
function removeRoleFromMember(member, role) {
    if (!member.roles.has(role)) {
        logger.warning(TAG, 'Request received to take ' + remove.name + ' role from ' +
