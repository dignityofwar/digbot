    ...
    vote(msg) {
        ...
        if (votes[msg.channel.id].options === false) {
            ...
            if (possitiveResponses.indexOf(msg.content.substring(6).toLowerCase()) !== -1) {
                if (!Object.prototype.hasOwnProperty.call(votes[msg.channel.id].count, 'yes')) {
            ...
            if (negativeResponses.indexOf(msg.content.substring(6).toLowerCase()) !== -1) {
                if (!Object.prototype.hasOwnProperty.call(votes[msg.channel.id].count, 'no')) {
                    votes[msg.channel.id].count.no = 0;
                }
                votes[msg.channel.id].count.no += 1;
                return `Thanks for voting ${msg.member.displayName}.`;
            }
            if (Object.prototype.hasOwnProperty.call(votes[msg.channel.id].count,
                msg.content.substring(6).toLowerCase())) {
            ...
            let message = `Sorry ${msg.member.displayName}, this is a multiple choice poll, `
                + 'acceptable answers are: ';
            let i = 0;
            for (i = 0; i < votes[msg.channel.id].options.length; i += 1) {
                if (i !== 0) {
                    message += ', ';
                }
...
function checkEnd(msg) {
    ...
    if (msg.author.id !== votes[msg.channel.id].msg.author.id) {
        return `Sorry ${msg.member.displayName}, votes can only be ended early by the person `
            + 'that called them.';
    }
...
function checkOptions(msg) {
    if (msg.content.indexOf('?') === -1 && msg.content.substring(6) !== 'x'
        && msg.content.substring(6) !== 'X') {
    ...
    if (msg.content.replace(/\s+/g, '').indexOf('?') + 1 < msg.content.replace(/\s+/g, '').length) {
        ...
        let options = msg.content.substring(msg.content.indexOf('?') + 3, msg.content.length - 1)
            .replace(/\s+/g, '');
        const n = options.replace(/[^,]/g, '').length + 1;
        let i = 0;
        for (i = 0; i < n - 1; i += 1) {
...
function endVoteManual(id) {
    ...
    for (x in votes[id].count) {
        message += `\n${x}: ${votes[id].count[x]} (${Math.round(100 * (votes[id].count[x] / total))}%)`;
        participation = true;
    }
    if (participation === false) {
        message += '\nAnd not a single vote was cast this day.';
...
function setupVote(msg, n) {
    const markIndex = msg.content.indexOf('?');
    let message = `**Poll Started!**: *${msg.content.substring(6)}*`
        + '\nTo vote use the !vote command, Examples: "!vote yes", "!vote blue"'
        + `\nThe vote will last 5 minutes or until ${msg.member.displayName} ends the vote early `
        + 'with: "!poll x"';
    votes[msg.channel.id] = {};
    votes[msg.channel.id].msg = msg;
    votes[msg.channel.id].question = msg.content.substring(6, markIndex + 1);
    ...
    logger.info(TAG, `Vote called by ${msg.member.displayName} in ${msg.channel.id}`);
    if (typeof n === 'number') {
        message += '\nThis is a multiple choice poll, only the specified replies will be counted.'
            + '\nOptions:';
        votes[msg.channel.id].options = [];
        let optionsString = msg.content.substring(markIndex + 3, msg.content.length - 1).replace(/\s+/g, '');
