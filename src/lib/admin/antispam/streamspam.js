...
module.exports = {
    execute(msg) {
        if (msg.guild.id !== config.get('general.server')) { return false; }
        if (msg.member.id === config.get('botUserID')) { return false; }
        const message = msg.content.replace(/\s+/g, '').toLowerCase();
...
function streamSpamAction(msg) {
    ...
    msg.channel.sendMessage(`${msg.member.displayName}, we have a specific channel for `
        + 'streams - #streams. I\'ve moved your message there.')
        .then(() => {
