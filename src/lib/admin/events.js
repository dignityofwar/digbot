...
function alert(eventObj) {
    ...
    if (server.getChannel('events') !== null) {
        server.getChannel('events').sendMessage(`${message}\n\n${mentions}`)
            .then(() => logger.info(TAG, 'Notified DIG Discord of event start'))
            .catch(err => logger.warning(TAG, `Was unable to notify DIG Discord of an event - error: ${err}`));
    }
    ...
    if (eventObj.millernotification === true) {
        if (server.getChannel('millerCommunityEvents') !== null) {
            server.getChannel('millerCommunityEvents').sendMessage(message)
                .then(() => logger.info(TAG, 'Notified Miller Community Discord of event start'))
                .catch(err => logger.warning(TAG,
                    `Was unable to notify Miller Community Discord of an event - error: ${err}`));
        ...
        if (server.getChannel('millerVSEvents') !== null) {
            server.getChannel('millerVSEvents').sendMessage(message)
                ...
                .catch(err => logger.warning(TAG, 'Was unable to notify Miller VS Discord of an event - '
                    + `error: ${err}`));
        } else if (config.util.getEnv('NODE_ENV') !== 'production') {
    ...
    for (let i = 0; i < eventObj.channels.length; i++) {
        if (eventObj.channels[i].type === 'text') {
            server.getGuild(config.get('general.server'))
                ...
                .catch(err => logger.warning(TAG, `Failed to create channel, error: ${err}`));
        } else {
...
function description(desc) {
    if (desc !== undefined) {
        desc = `*${desc}*`; // eslint-disable-line no-param-reassign
        return desc;
    }
    return '';
}
...
function format(num) {
    if (num < 10) {
        num = `0${num.toString()}`; // eslint-disable-line no-param-reassign
    }
    return num;
}
